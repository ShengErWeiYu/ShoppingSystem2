...
func InitRouters() {
	...
	user := r.Group("/user")
	{
		...
		goods := r.Group("/user/goods")
		{
			...
			goods.POST("/like/search", middleware.JWTAuthMiddleware(), controller.SearchGoodInLike, homeHandler) //SearchGoodInLike1111111111111111111111
			//商家用户的商品操作
		...
		user.POST("/checkorder", middleware.JWTAuthMiddleware(), controller.CheckOrder, homeHandler) //CheckOrder
	}
